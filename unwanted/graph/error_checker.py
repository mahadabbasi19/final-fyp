...
class StaticAnalyzer:
    ...
    def _check_unused_imports(self, code: str, lines: List[str], comment_map: Dict[int, bool]):
        ...
        for i, line in enumerate(lines, 1):
            ...
            if match:
                ...
                if full_import.endswith('.*'):
                    # Skip wildcard imports - can't check usage
                    continue
        ...
        # Get code after imports for usage checking (skip import lines)
        code_lines_no_imports = []
        for j, line in enumerate(lines, 1):
            stripped = line.strip()
            if not stripped.startswith('import '):
                code_lines_no_imports.append(line)
        code_without_imports = '\n'.join(code_lines_no_imports)
        
        # Common Java classes that are implicitly used or commonly needed
        # These are often used in ways that are hard to detect via regex
        commonly_implicit_classes = {
        ...
        for line_num, full_import, class_name, is_static in imports:
            # Skip commonly used classes that may be used in subtle ways
            if class_name in commonly_implicit_classes:
                continue
            
            # Build pattern to find usage of the imported class
            usage_found = False
            
            # Simple word boundary check - if the class name appears as a word anywhere
            # This is the most reliable check
            simple_pattern = re.compile(r'\b' + re.escape(class_name) + r'\b')
            
            if simple_pattern.search(code_without_imports):
                usage_found = True
